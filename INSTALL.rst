...
* ``DB_NAME``: name of the database for the project. Defaults to ``openpersonen``.
* ``DB_USER``: username to connect to the database with. Defaults to ``openpersonen``.
* ``DB_PASSWORD``: password to use to connect to the database. Defaults to ``openpersonen``.
...
2. Start the database and web services:
   ...
   .. code-block:: bash
       ...
       $ docker-compose up -d
       Starting openpersonen_db_1 ... done
       Starting openpersonen_web_1 ... done
   ...
   .. code-block:: bash
       ...
       $ docker logs -f openpersonen_web_1
...
3. Create an admin user and load initial data. If different container names
   ...
   .. code-block:: bash
       ...
       $ docker exec -it openpersonen_web_1 /app/src/manage.py createsuperuser
       Username: admin
       ...
       Superuser created successfully.

       $ docker exec -it openpersonen_web_1 /app/src/manage.py loaddata admin_index groups
...
environment variables. See ``src/openpersonen/conf/docker.py`` for
all settings.

.. code-block:: bash

    $ docker build -t openpersonen
    $ docker run \
        -p 8000:8000 \
        -e DATABASE_USERNAME=... \
        -e DATABASE_PASSWORD=... \
    ...
    $ docker exec -it openpersonen /app/src/manage.py createsuperuser
