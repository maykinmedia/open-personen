...
expected_headers = [
    ...
    'Reden beëindigen nationaliteit',
...
class Command(BaseCommand):
    ...
    def handle(self, **options):
        ...
        for index, row in enumerate(data['Toelichting'][33:286]):
            if len(row) > 0:
                if row[-1] == 'RNI':
                    assert expected_headers[index] == row[-2], f'At index {index}: {row[-2]} does not match expected {expected_headers[index]}'
                else:
                    assert expected_headers[index] == row[-1], f'At index {index}: {row[-1]} does not match expected {expected_headers[index]}'
            else:
                assert [] == row, f'At index {index}: {row} does not match expected {expected_headers[index]}'
